...
def read_gml_adversarial(DIR, util_range=[1, 4], demand_range=[1, 2], seed=4142, adversarial_node_util=10):
    """
    ...
    for node in G.nodes:
        # random.seed(seed)
        u = random.randint(util_range[0], util_range[1])
        utils.update({node: u})
        # random.seed(seed)
        d = random.randint(demand_range[0], demand_range[1])
        demand.update({node: d})

    if DIR[-7:] == 'ibm.gml':
        # Construct adversarial example by hand for ibm
        # random.seed(seed)
        adv_node = random.randint(0, G.number_of_nodes())
        utils.update({adv_node: adversarial_node_util})
        demand.update({adv_node: 1})

        # This one change drops acc of ratio when compared to opt by 20% for seed 404
        # 2: (1,1) => 968/1008
        # 2: (1,2) => 972/1017
        # 2: (1,3) => 833/1055
        # 2: (1,4) => 833/1055
...
def r_graph(n, edge_prob, util_range=[1, 4], demand_range=[1, 2], seed=None):
    ...
    :param util_range: range to generate random utility from (inclusive)
    :param demand_range: range to generate random demand from (inclusive)
    :param seed: If random seed, set seed
    :return: Random random graph with util, demand set for each node.
    """
    if seed:
        random.seed(seed)
