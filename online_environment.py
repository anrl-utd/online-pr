...
# are vectors of length NumNodes(G), actions are choosing two nodes to recover (an index into 
# the list of all 2-permutations of nodes), and reward is the sum of the utilities 
# of all functional nodes.
class Environment:
    def __init__(self, G, independent_nodes, resources):
        ...
        # G_constant is a graph that stays constant across episodes so when we reset we do it cleanly
        # by copying G_constant into G 
        ...
        for adj_node in adjacent_to:
            for node in range(self.number_of_nodes):
                if node in self.G_constant.neighbors(adj_node) and node != adj_node and demand[node] > 0:
                    ...
                    # if we can fully recover this node at a given time step, then we may 
                    # start allocating resources to it's neighbors
    ...
    def random_action(self, return_indices=False):
        ...
        # we may want to return the list of random actions for the 1-\epsilon case (in RL)
        if return_indices:
